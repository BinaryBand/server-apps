...
class TestReconcilerErrorHandling:
    ...
    def test_reconcile_once_handles_runtime_error_during_permissions(self, state_root_tmp, monkeypatch):
    ...
    def test_reconcile_once_handles_runtime_error_during_post_start(self, state_root_tmp, monkeypatch):
...
class TestHealthChecksErrorHandling:
    ...
    def test_run_runtime_health_checks_handles_multiple_failures(self):
        ...
        with patch("src.toolbox.core.config.rclone_remote", return_value="pcloud"):
            with patch("src.toolbox.docker.health.wait_for_container_exec") as mock_exec:
