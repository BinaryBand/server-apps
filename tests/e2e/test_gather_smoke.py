        ...
        subprocess.run(
            [
                ...
                (
                    ...
                    f"/target/data/{logical_name}.txt"
                ),
            ],
            check=True,
        )
        ...
        probe = subprocess.run(
            [
                ...
                *docker_args,
                ...
                "-lc",
                (
                    f"test -d '{container_path}' && find '{container_path}' "
                    f"-mindepth 1 -print -quit "
                    f"| grep -q ."
                ),
            ],
            check=False,
...
def test_gather_mounts_populated_stage_roots(gather_env, monkeypatch) -> None:
    ...
    monkeypatch.setattr("src.infra.docker.rclone.rclone_sync", fake_rclone_sync)
    monkeypatch.setattr(
