...
from src.infra.config import rclone_version

_MEDIA_MOUNT_PATH = "/media"


def _rclone_image() -> str:
    return f"rclone/rclone:{rclone_version('latest')}"
...
def _mount_path_exists() -> bool:
    return _docker_exec_ok(["test", "-d", _MEDIA_MOUNT_PATH]).returncode == 0
...
def _command_exists(command: str) -> bool:
    return _docker_exec_ok(["sh", "-lc", f"command -v {command} >/dev/null 2>&1"]).returncode == 0
...
def _try_unmount_with(command: str, *args: str) -> bool:
    if not _command_exists(command):
        return False
    return _docker_exec_ok([command, *args, _MEDIA_MOUNT_PATH]).returncode == 0
